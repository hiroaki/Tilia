...
プロファイル識別子は Phloem へ送信する前に小文字へ正規化され、選択 UI の設定では組み込み識別子 `car` / `bike` / `foot` のみを受け付けます。未知の値は Phloem リクエスト body の生成時点で reject されます。
既知のプロファイルは現在、選択 UI で `Car` / `Bike` / `Foot` の直接ラベルで表示されます。ラベル整形は表示専用として 1 箇所に集約し、将来的に I18n 翻訳へ差し替えてもリクエスト契約を変えない設計方針です。
