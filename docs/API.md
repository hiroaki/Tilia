...
| `profileOptions` | `string[]` | `[defaultProfile, "car", "bike", "foot"]` | Allowed profile values in the profile selector (`car`, `bike`, `foot`). Unknown values are ignored. |
...
Profile identifiers are normalized to lowercase before requests are sent to Phloem, and only the built-in identifiers `car`, `bike`, and `foot` are accepted by the selector setup. Unknown profile values are rejected when building the Phloem request body.
